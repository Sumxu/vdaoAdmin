...
import {httpExport} from '@/utils/http/exportIndex'
 ...
 * post
 */
export function post<T, P>(url: string, data: T,config): Promise<Result<P>> {
  return http.post<T, Result<P>>(url, { data: data },config);
}
