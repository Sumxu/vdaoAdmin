import { type Result, deleteRequest, get, post, put } from "../base";
/**
 * 分页查询
 * @param query .
 * @returns .
 ...
 * @returns
 */
export function getGoogleUrl<T>(query?: any): Promise<Result<T>> {
  return get("/sys/user/query/googleUrl", query);
}
 ...
 * @returns
 */
export function resetGoogleCode<T>(data: any): Promise<Result<T>> {
  return post("/sys/user/resetGoogleCode", data);
}
