...
export interface FormField {
   ...
   * 字段类型
   */
  type: "input" | "select" | "date" | "number" | "radio";
  /**
   * 表达label
   */
  label: string;
...
const pageData: any = reactive({ formData: props.initialFormData });
/**
 * 获取默认值
 */
const _getDefaultForm = () => {
  ...
  fields.forEach((field: FormField) => {
    obj[field.prop] = field.default ?? null;
  });
  return obj;
};
...
watch(
  () => props.initialFormData,
  newVal => {
    Object.assign(pageData.formData, newVal);
    _searchForm(pageData.formData);
  },
  { deep: true }
);
