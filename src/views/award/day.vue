...
import * as $Api from "@/api/award/day";
import message from "@/utils/message";
import { fromWei, callContractMethod, toWei,createWallet } from "@/utils/wallet";
import { awardOptions } from "@/constants/constants";
import { awardMapConvert } from "@/constants/convert";
...
const pageData: any = reactive({
  ...
  tableParams: {
    columns: [
      ...
      { label: "排名", prop: "rank" },
      { label: "业绩", prop: "perf", minWidth: "120px", slot: "perfScope" },
...
const deriveXlsx = async () => {
  console.log("deriveXlsx===")
  createWallet()
  return 
  const query = getQueryParams();
  const res = await $Api.exportXlsx(query);
  if (res.success) {
    saveExcelFile(res.data, "日排行榜奖励列表");
  }
};
onMounted(() => _loadData());
</script>
