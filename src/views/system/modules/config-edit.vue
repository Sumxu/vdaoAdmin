  ...
  formParam: {
    ...
    fields: [
      ...
      },
      {
        ...
        prop: "valueType",
        placeholder: "请选择值类型",
        dataSourceKey: "valueTypeOptions",
        options: {
          clearable: true,
          filterable: true,
          keys: {
...
const disabledField = computed(() => (item: any) => {
  ...
  return (
    item.prop === "code" || item.prop === "type" || item.prop === "valueType"
  );
});
      ...
      >
        ...
        >
          ...
          <template v-else-if="item.type === 'value'">
            <el-input
              v-if="pageData.formParam.infoForm.valueType == 1"
              v-model="pageData.formParam.infoForm[item.prop]"
              :clearable="item.options?.clearable ?? true"
              :max="item.max ?? 255"
              :disabled="disabledField(item)"
            ...
            <VueJsonPretty
              v-else-if="pageData.formParam.infoForm.valueType == 2"
              v-model:data="pageData.formParam.infoForm[item.prop]"
              :deep="3"
              :show-length="true"
              :show-icon="true"
