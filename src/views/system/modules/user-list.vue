...
const _loadData = (page?: number) => {
  ...
  $userApi
    ...
    .then((res: any) => {
      if (res.code === 200) {
        pageData.tableParam.list = res.data.records;
        pageData.tableParam.pagination.total = Number(res.data.total);
      }
    })
    .finally(() => {
...
const queryOrg = () => {
  ...
  orgQueryList(query).then((res: any) => {
    if (res.code === 200) {
      pageData.dataSource.orgList = res.data;
    }
  });
};
const queryRoles = () => {
  roleQueryList({ enabled: 1 }).then((res: any) => {
    if (res.code === 200) {
      pageData.dataSource.roleList = res.data;
    }
  });
};
...
const handleResetGoogle = (id: any) => {
  message.confirm("是否重置该账号谷歌二维码").then(() => {
    $userApi.resetGoogleCode({ id: id }).then((res: any) => {
      if (res.success) {
...
const handleGetGoogleUrl = (username: any) => {
  $userApi.getGoogleUrl({ username }).then(res => {
    googleData.dialogVisible = true;
    googleData.qrCodeValue = res.data;
  });
};

const googleData: any = reactive({
  dialogVisible: false,
  qrCodeValue: ""
});
...
<template>
  <el-card :shadow="'never'">
    <template #default>
      ...
      >
        ...
        <template #operation="{ row }">
          <div class="flex justify-center items-center">
            <el-link type="success" @click="handleGetGoogleUrl(row.username)"
              >谷歌二维码</el-link
            >
            ...
            <el-dropdown v-show="row.isManage !== 1">
              ...
              <template #dropdown>
                <el-dropdown-menu>
                  ...
                  >
                  <el-dropdown-item @click="handleResetGoogle(row.id)"
                    >重置谷歌二维码</el-dropdown-item
                  >
                </el-dropdown-menu>
              </template>
            </el-dropdown>
      ...
      <el-dialog
        v-model="googleData.dialogVisible"
        title="谷歌二维码"
        width="200"
      >
        <VueQrcode
          :value="googleData.qrCodeValue"
          type="image/png"
        ...
        <template #footer>
          <div class="dialog-footer">
            <el-button @click="googleData.dialogVisible = false"
              >关闭</el-button
            >
          </div>
        </template>
